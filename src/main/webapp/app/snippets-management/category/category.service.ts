...
export class CategoryService {
  ...
  public filterCategory(query: string, res: EntityArrayResponseType): ICategory[] {
    const listCategoriesResponse: ICategory[] | null = res.body;
    const filtered: ICategory[] = [];

    if (listCategoriesResponse != null) {
      for (let i = 0; i < listCategoriesResponse.length; i++) {
        const category: ICategory = listCategoriesResponse[i];
        if (category.categoryDesc && category.categoryDesc.toLowerCase().startsWith(query.toLowerCase())) {
